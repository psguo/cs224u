...
def run_small_glove_evals():
    ...
    for iteration in [10, 100, 200]:
        ...
        scores[iteration] = full_word_similarity_evaluation(giga20_glv)['Macro-average']
    return scores
...
def dice(u, v):
    ...
    return 1 - 2 * np.sum(np.min([u, v], axis=0)) / np.sum(u+v)
...
def ttest(df):
    ...
    P_ij = df / np.sum(df)
    I, J = df.shape
    P_i_star = np.sum(P_ij, axis=1, keepdims=True)
    P_j_star = np.sum(P_ij, axis=0, keepdims=True)

    result = (P_ij-P_i_star*P_j_star)/np.sqrt(P_i_star*P_j_star)
    return result
...
def subword_enrichment(df, n=4):
    ...
    for word in df.index:
        counts.append(vsm.character_level_rep(word,df_ngram,n))
    ...
    ##### YOUR CODE HERE
    df_ngram_level = pd.DataFrame(counts, index=df.index)
    df += df_ngram_level
...
def test_original_system(run_original_system):
    giga20 = pd.read_csv(
        os.path.join(VSM_HOME, "giga_window5-scaled.csv.gz"), index_col=0)
    results = run_original_system(giga20)
    return results
    # print(results)
    # print('Macro-average:', results.loc['Macro-average'])

if 'IS_GRADESCOPE_ENV' not in os.environ:
    display(test_original_system(run_original_system))
